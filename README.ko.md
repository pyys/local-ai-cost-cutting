...
- **모델이 한 카드에 안 들어가 텐서 병렬이 필요하면** 전제가 완전히 달라진다. 이종 카드 간 텐서 병렬은 느린 카드에 맞춰진다 (다만 "NVLink급 인터커넥트가 필요하다"는 부분은 이후 측정에서 **디코드에 한해 과한 서술**로 드러났다 -> [별도 저장소](https://github.com/pyys/layer-tensor-parallel-bench))
...
| **모델이 한 카드에 안 들어가 여러 장에 쪼개야 한다** | [layer-tensor-parallel-bench](https://github.com/pyys/layer-tensor-parallel-bench) (별도 저장소) |
