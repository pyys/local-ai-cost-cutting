...
- **모델이 한 카드에 안 들어가 레이어 병렬/텐서 병렬이 필요하면** 전제가 달라진다. 검증 목표도 실험 설계도 달라지므로 이 문서가 다루는 범위 밖이다. 참고할 자료 중 하나로 [layer-tensor-parallel-bench](https://github.com/pyys/layer-tensor-parallel-bench)가 있다
...
| **모델이 한 카드에 안 들어가 레이어/텐서 병렬을 검토해야 한다** | [layer-tensor-parallel-bench](https://github.com/pyys/layer-tensor-parallel-bench) (별도 저장소) |
