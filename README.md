...
- **If the model does not fit on one card and layer or tensor parallelism is needed**, the premises change. The goals and the experimental design both differ, so it falls outside what this document covers. One reference among others: [layer-tensor-parallel-bench](https://github.com/pyys/layer-tensor-parallel-bench)
...
| **The model does not fit on one card, so layer or tensor parallelism is on the table** | [layer-tensor-parallel-bench](https://github.com/pyys/layer-tensor-parallel-bench) (separate repository) |
